...
function searchInFuseResults(dataObject, item, indexJSON) {
  let out = "";
  try {
    const which_key = indexJSON.keys.findIndex(
      (element) => element.id === item.key
    );
    const index_array = indexJSON.records[0].$[which_key];
    const found_matches = [];
    ...
    const which_instance = found_matches.findIndex(
      (elem) => elem === item.refIndex
    );

    const path = searchInLib(dataObject, which_key, item.value);
        ...
        try {
          for (
            ...
            for (
              ...
              module_class_i <
              dataObject.libraries[lib_i].modules[lib_module_i].classes.length;
              module_class_i++
            ) {
              if (
                name_to_find ===
