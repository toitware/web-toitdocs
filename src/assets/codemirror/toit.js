...
(function (mod) {
  ...
  CodeMirror.defineMode("toit", function (config) {
    ...
    var keywords = makeJsObject("assert|and|or|not|if|for|else|try|finally|" +
    ...
    // If all else fails, we assume that we are implicitly inside a function
    // like "main".
    function tryImpliedTopLevelFunction(stream, state) {
      state.context.push([tokenizeFunctionBody, 0]);
      state.subState.push(null);
      return "null";
    }
